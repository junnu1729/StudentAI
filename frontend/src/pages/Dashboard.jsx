...
import { FileText, Sparkles, CheckCircle, XCircle, Trophy, BookOpen } from 'lucide-react'
...
export default function Dashboard() {
  ...
  async function handleSummarize() {
    ...
    try {
      ...
      setSummary('Error: ' + (err.response?.data?.error || 'Failed to summarize'))
    }
  ...
  async function handleGenerateQuiz() {
    ...
    try {
      const res = await generateQuiz({ note_id: selectedNote, num_questions: numQ })
      setQuiz(res.data)
    } catch (err) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10 space-y-8">
      ...
      <div className="card p-5">
        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select a Note</label>
        <select className="input" value={selectedNote} onChange={e => { setSelectedNote(e.target.value); setSummary(''); setQuiz(null); setResult(null) }}>
          <option value="">-- Choose a note --</option>
          {notes.map(n => <option key={n.id} value={n.id}>{n.original_name} ({n.subject})</option>)}
        </select>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={handleSummarize} disabled={!selectedNote || summaryLoading} className="btn-primary text-sm flex items-center gap-2">
        ...
        {summary ? (
          ...
          <p className="text-sm text-gray-400 text-center py-6">Select a note and click Generate Summary</p>
        )}
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleGenerateQuiz} disabled={!selectedNote || quizLoading} className="btn-primary text-sm flex items-center gap-2">
        ...
        {quiz && !result && (
          <div className="space-y-5">
                  ...
                  {q.options.map((opt, j) => {
                    ...
                    return (
                      <label key={j} className={`flex items-center gap-3 p-2.5 rounded-lg cursor-pointer transition-colors ${
                        ...
                        <input type="radio" name={`q${i}`} value={letter} checked={answers[i] === letter} onChange={() => setAnswers({ ...answers, [i]: letter })} className="hidden" />
                        <span className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs font-bold flex-shrink-0 ${answers[i] === letter ? 'border-blue-500 bg-blue-500 text-white' : 'border-gray-300 dark:border-gray-600'}`}>{letter}</span>
            ...
            <button onClick={handleSubmitQuiz} disabled={Object.keys(answers).length < quiz.questions.length || submitting} className="btn-primary w-full flex items-center justify-center gap-2">
        ...
        {result && (
          <div className="space-y-4">
            ...
            {result.results.map((r, i) => (
              <div key={i} className={`p-4 rounded-lg border ${r.is_correct ? 'border-green-200 bg-green-50 dark:bg-green-900/20 dark:border-green-800' : 'border-red-200 bg-red-50 dark:bg-red-900/20 dark:border-red-800'}`}>
                <div className="flex items-start gap-2">
                  {r.is_correct ? <CheckCircle className="w-4 h-4 text-green-500 mt-0.5 flex-shrink-0" /> : <XCircle className="w-4 h-4 text-red-500 mt-0.5 flex-shrink-0" />}
                  <div>
        ...
        {!quiz && !result && <p className="text-sm text-gray-400 text-center py-6">Select a note and click Generate Quiz</p>}
      </div>
